...
if (isTRUE(getOption(dabtab.dabtab.authenticated)) && !result$userPermission == "Allow") {
  if (nrow(result) > 0) {
    userUUID <- result$userUUID
    folder_name <- "userUUID"  # Replace with the desired folder name
    tryCatch({
      put_object(data = NULL, object = folder_name, s3 = s3)
    }, error = function(e) {
      cat("Error 101: Folder creation has failed for user ", userUUID, "\n", file = "error.log", append = TRUE)
    })
  } else {
    cat("Error: Can't create folder for the user ", userUUID, "\n", file = "error.log", append = TRUE)
  }
} else if (isTRUE(getOption(dabtab.dabtab.authenticated)) && !result$userPermission == "Suspend") {
  cat("Error 102: User Account ID ", userUUID, " has been suspended. See more details.\n", file = "error.log", append = TRUE)
} else if (isTRUE(getOption(dabtab.dabtab.authenticated)) && !result$userPermission == "Prohibit") {
  cat("Error 103: User Account ID ", userUUID, " has been permanently terminated. See more details.\n", file = "error.log", append = TRUE)
} else {
...
thumbnail_img <- image_scale(img, "100x100")  # Adjust the size as needed

# Save the thumbnail to a file
thumbnail_path <- "path/to/your/thumbnail.jpg"
image_write(thumbnail_img, path = thumbnail_path)

# Display the thumbnail in your Shiny app
thumbnail_img

tag$div(box then append thumnail to it)

c<-list()

#by default data will be loaded from user folder in AWS S3 bucket

obserEvent(input$uploadData){
  check_guest_infor
}

obserEvent("input$viewShared")



obserEvent(input$check){
  check_guest_infor
}
...
create_pair_folder(hostname, guestname) {
  loop through each row in the table 
  take infor from column sharewith 

  query from database table/ which infact in the far left of user screen
  if isTrue (check_guest_infor())

 folderName = comb("usernameID"+ "-"+"sharedwithList")
 add a new folder to the bucket of AWS S3
 else {
"the inputs are invalid.Please recheck."
 }
...
check_guest_infor(){
  loop through the sharewith list 
  query from database

  if there is any user is not in the dabtab database 

  showModal(modalDialog(
    "Please check these users + list of users not a part of DabTab. Remove from the share list or request these users to register to proceed."
    ))
}

# Define a policy document to deny access to a specific folder
generate_deny_policy <- function ()(


 str<- paste0(create_pair_folder())
...
# Create the policy
policy <- isolate(s3_create_policy("DenyAccessToFolder", policy_document))

# Attach the policy to the user
s3_put_user_policy<-isolate(user_name = "user-to-be-restricted", policy_arn = policy$Policy$Arn)


)

generate_allow_policy <- function ()(
...
# Create the policy
policy <- s3_create_policy("AllowAccessToFolder", policy_document)

# Attach the policy to the user
s3_put_user_policy(user_name = "user-to-be-restricted", policy_arn = policy$Policy$Arn)

)


      <- lapply(emailList(), function(email) {
      ...
      do.call(tagList, tagElements)

applyAll()<-

obserEvent(if(...#conditions satisfied))(applyAll)

create_userSharedDir(
list(username-)
)


emailList <- reactiveVal(character())
  observeEvent(input$addTag, {
    newEmail <- isolate(input$emailInput)
    if (newEmail != "" && !newEmail %in% emailList()) {
      emailList(c(emailList(), newEmail))
    }
  })

#this is to store all the values in the 
sharedwithList <- list()

restrictAcessList <- list()


for each i in restrictAcessList {

}


  observe({
    output$tagContainer <- renderUI({
  ...
  observeEvent(input$copyPrev){
    if length (tagElementsPrev) >0 {
      list rowAvail = !input$clickedRow in rownames
            taglist()
            "Copy from Row" & rowavial
     tagElementsPrev 
    tagElementsPrev
...
sharedTable <- column(
  singleton(tags$head(tags$script(HTML('
    $(document).on("click", ".row-clickable", function(){
      ...
      Shiny.setInputValue("selected_row", row);
    });
  '))),
  tableOutput("myTable"),
)
...
nrows <- length(obj_list)
rownames <- length(obj_list)


  output$myTable <- renderTable({

    #define column names and row names of matrix
colnames(tab) <- c('Data', 'ShareWith', 'Restrict','Level')


with level column it's a list of folder, object level

obserEvent (input$sharedApply){
...
rownames(tab) <- 
    # Example table
    data.frame(ID = 1:5, Value = letters[1:5])
  },)


  output$clickedRow <- renderText({
    req(input$selected_row)
    paste("Clicked row:", input$selected_row)
  })
...
#the idea behind is when users enter the email that they want to share data with append to the container hence the row
#the number of rows is equal to number of objects in the folder
#create a new folder with the package which a pair of the host-guest(i.e., user1-user2)
...
there cancel button there as well 
...
webhook logic here send notifications to user


validate_sharedUser(input$shareWith)

validate_sharedUser(useremail) {


  check if the user email exists in the database if not notify users 
...
custom_template which means build from scratch

builtin_template which means build from provided template
...
there would be two options one is set theme based on the list themeSelector

or custom theme and apply to new session if you will
...
ui <- fluidPage(
  useShinyjs(),
  actionButton("toggleThemeSelector", "Toggle Theme Selector"),
  shinythemes::themeSelector(),  # Add this somewhere in the UI
  textOutput("selectedTheme")
)

server <- function(input, output, session) {
  observeEvent(input$toggleThemeSelector, {
    if (input$toggleThemeSelector %% 2 == 1) {
      runjs("$('.theme-selector').show();")  # Show the theme selector
    } else {
      runjs("$('.theme-selector').hide();")  # Hide the theme selector
    }
  })

  output$selectedTheme <- renderText({
    if (!is.null(input$shinythemes_theme)) {
      # Perform actions based on the selected theme
      paste("Selected theme:", input$shinythemes_theme)
    }
  })
}
...
>>>>>>>>>>>>>>>>>>CHECK IF BOOTSWATCH IS WORKING
...
this is to navigate between sessions


////////////////////
//keyboard shortcuts to call specific functionality
/////////////


////////////////////
//hover will display functionality you can turn off if you wish
/////////////
...
[[[[[[   custom formula         ]]]]]] >>>>read eval execute code
...
one func -> one range>>>ok>>>one result>>>>paste to one/multiple ranges


one func -> multiple ranges>>>ok>>>multiple results>>drag to range or hold ctr key then click onto the box to paste



have to manipulate each result otherwise it won't work


mutiple funcs -> one range>>>ok

mutiple funcs -> multiple ranges>>>check if match numbers won't work yes means from top down and left unmatch rows cancel out
...
#Function to dynamically render RMarkdown reports
   observe({
    ...
    report_file <- switch(
      selected_report,
      html_report <- file.path(instDir, "app/www/reports/dabtab_html.Rmd"),
      pdf_report <- file.path(instDir, "app/www/reports/dabtab_pdf.Rmd"),
      word_report <- file.path(instDir, "app/www/reports/dabtab_word.Rmd"),
      pptx_report <- file.path(instDir, "app/www/reports/dabtab_pptx.Rmd"),
      shinyHtml_report <- file.path(instDir, "app/www/reports/dabtab_shiny_html.Rmd"),
      shinyPptx_report <- file.path(instDir, "app/www/reports/dabtab_shiny_pptx.Rmd"),
      error = "Invalid Format"
    )
    report_output <- renderUI({
      rmarkdown::render(report_file)
      includeHTML(report_file) # Embed the rendered HTML report
    })  
