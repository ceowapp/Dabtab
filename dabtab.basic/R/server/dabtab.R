...
launch <- function(package = "dabtab.basic", run = "viewer", state, ...) {
  ...
  suppressPackageStartupMessages(
    shiny::runApp(system.file("app", package = package), host="192.168.xx.xx",port=8080, launch.browser = run, ...)
  )
}
...
#' \dontrun{
#' dabtab.basic()
#' dabtab.basic("https://github.com/aigeniusxadmin/dabtab-shinyapp")
#' dabtab.basic("viewer")
#' }
#' @export
...
#' @param md Data description in text format
#' @param path Path to a text file with the data description in text format
#'
#' @examples
#' if (interactive()) {
...
#' Function to exclude outliers from dataset based on Z
...
#' Function to exclude outliers from dataset based on IQR
#' @details Return two datasets before and after removing outliners
#' @param df Dataframe
#' @param var_name Column
#' @examples
#'   iqr_outlier(df, var_name="column A")
#' @export
...
#' Function to exclude outliers from dataset based on MAD
#' @details Return two datasets before and after removing outliners
#' @param df Dataframe
#' @param var Column
#' @examples
#'   mad_outlier(df, var="column A")
#' @export
...
#' Function to exclude outliers from dataset based on LOF
#' @details Return two datasets before and after removing outliners
#' @param df Dataframe
#' @param var Column
#' @param k Number of nearest neighbors
#' @examples
#'   lof_outlier(df, var="column A", k=10)
#' @export
...
box_and_whisker_plot_1 <- function(df, var1, var2) {
  if (nrow(df) < 20 || !is.numeric(df[[var2]])) {
    sendSweetAlert(
      session = session,
      title = "Warning !!!",
      text = "There is not enough data or invalid data type to generate the Box and Whisker Plot.",
      type = "warning"
    )
    return(NULL)
  }
...
#' @details Return the scatter plot based on input
#' @param df Data frame
#' @param var1 Variable as x axis
#' @param var2 Variable to plot against
#' @examples
#'   scatter_plot(data, "Column A", "Column B")
#' @export
scatter_plot <- function(df, var1, var2) {
  if (nrow(df) < 20 || !is.numeric(df[[var2]])) {
    sendSweetAlert(
      session = session,
      title = "Warning !!!",
      text = "There is not enough data or invalid data type to generate the Scatter Plot.",
      type = "warning"
    )
    return(NULL)
  }
...
#' @details Return the histogram plot based on input
#' @param df Data frame
#' @param var Variable as x axis
#' @examples
#'   histogram_plot(data, "Column A")
#' @export
...
#' @details Return the means plot based on input
#' @param df Data frame
#' @param var1 Variable as x axis
#' @param var2 Variable as x axis
#' @param var3 Variable as x axis
#' @examples
#'   means_plot(data, "Column A", "Column B", "Column C")
#' @export
...
#' @details Return the density plot based on input
#' @param df Data frame
#' @param var1 Variable as x axis
#' @param var2 Variable to plot against
#' @examples
#'   density_plot(data, "Column A", "Column B")
#' @export
density_plot <- function(df, var1, var2) {
  if (nrow(df) < 20 || !is.numeric(df[[var2]])) {
    sendSweetAlert(
      session = session,
      title = "Warning !!!",
      text = "There is not enough data or invalid data type to generate the Density Plot.",
      type = "warning"
    )
    return(NULL)
  }
...
#' @details Return the table containing misspelling words
#' @param df Data frame
#' @param var Variable as x axis
#' @examples
#'   check_spelling(data, "Column A")
#' @export
...
#' @details Return the table containing text inputs and converted numbers
#' @param df Data frame
#' @param var Variable as x axis
#' @examples
#'   text_to_num(data, "Column A")
#' @export
text_to_num <- function(df, var) {
  ...
  return(list(data=df,converted=converted, invalid=invalid))
}

#' @details Return the table containing number inputs and converted texts
#' @param df Data frame
#' @param var Variable as x axis
#' @examples
#'   num_to_text(data, "Column A")
#' @export
num_to_text <- function(df, var) {
  ...
  for (i in 1:nrow(df)) {
    ...
    if (!is.numeric(df[[var]][i]) || is.na(df[[var]][i])) {
      # mark the row as invalid and increment the counter
      df$invalid_input[i] <- ifelse(!is.numeric(df[[var]][i]) || is.na(df[[var]][i]), "invalid", "")
      num_invalid <- num_invalid + 1
    } else {
      # convert the number to text and mark the row as converted
      df$num_to_text[i] <- three_digit_to_text(df[[var]][i])
...
#' @details Return the data table for data frame if there are more than 1000 rows or 20 columns, enable pagination and else
#' @param df Data frame
#' @examples
#'   render_custom_datatable(data)
#' @export
...
#' @details Return the data table
#' @param df Data frame
#' @examples
#'   render_custom_datatable_1(data)
#' @export
...
#' @details Return the filtered data frame based on filters
#' @param data Data frame
#' @param col_name Column
#' @param search_term Search inputs
#' @param rows Row
#' @examples
#'   filtered_df(data, "Column A", "above value X", rows)
#' @export
