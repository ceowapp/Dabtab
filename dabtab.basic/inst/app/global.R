...
# options(dabtab.autosave = c(1, 5))

## set volumes if sf_volumes was preset (e.g., on a server) or
## we are running in Rstudio or if we are running locally
if (isTRUE(getOption("dabtab.sf_volumes", "") != "") ||
  isTRUE(getOption("dabtab.shinyFiles", FALSE)) ||
  isTRUE(Sys.getenv("RSTUDIO") != "") ||
  isTRUE(Sys.getenv("SHINY_PORT") == "")) {
  if (isTRUE(getOption("dabtab.sf_volumes", "") != "")) {
    sf_volumes <- getOption("dabtab.sf_volumes")
    if (length(names(sf_volumes)) == 0) {
      warning("\nOption dabtab.sf_volumes should be a named vector set in .Rprofile\n\n")
      options(dabtab.sf_volumes = "")
    } else if (any(sapply(sf_volumes, function(x) !dir.exists(x)))) {
      warning("\nOne or more directories listed in option dabtab.sf_volumes do not exists. Please fix the option in .Rprofile and restart dabtab.\n\n")
      options(dabtab.sf_volumes = "")
    }
    rm(sf_volumes)
  }
  ...
  if (isTRUE(getOption("dabtab.sf_volumes", "") == "")) {
    sf_volumes <- c(Home = dabtab.data::find_home())
    ...
    Dropbox <- try(dabtab.data::find_dropbox(), silent = TRUE)
    if (!inherits(Dropbox, "try-error")) {
      sf_volumes <- c(sf_volumes, Dropbox = Dropbox)
    }
    GoogleDrive <- try(dabtab.data::find_gdrive(), silent = TRUE)
    ...
    options(dabtab.sf_volumes = sf_volumes)
  }
  options(dabtab.shinyFiles = TRUE)
} else {
  options(dabtab.shinyFiles = FALSE)
}

## determining how dabtab was launched
## should this be set in global?
if (is.null(getOption("dabtab.launch"))) {
  ...
  if (exists(".rs.readUiPref")) {
    if (is.null(.rs.readUiPref("shiny_viewer_type"))) {
      ...
      # options(dabtab.launch = "external")
...
init_data <- function(env = r_data) {
  ...
  datasetlist <- c()
  df_names <- getOption("dabtab.init.data")
  ...
  for (dn in df_names) {
    if (file.exists(dn)) {
      ...
      df <- data(list = dn, package = "dabtab.data", envir = environment()) %>% get()
      r_info[[paste0(dn, "_lcmd")]] <- glue::glue('{dn} <- data({dn}, package = "dabtab.data", envir = environment()) %>% get()\nregister("{dn}")')
    }
    env[[dn]] <- df
...
if (getOption("dabtab.jupyter", default = FALSE)) {
  options(dabtab.local = FALSE)
  options(dabtab.report = getOption("dabtab.report", default = TRUE))
  ## no limit to file size when launched through jupyter
  options(shiny.maxRequestSize = getOption("dabtab.maxRequestSize", default = -1))
} else if (Sys.getenv("SHINY_PORT") == "") {
  options(dabtab.local = TRUE)
  options(dabtab.report = getOption("dabtab.report", default = TRUE))
  ## no limit to file size locally
  options(shiny.maxRequestSize = getOption("dabtab.maxRequestSize", default = -1))
} else {
  options(dabtab.local = FALSE)
  options(dabtab.report = getOption("dabtab.report", default = FALSE))
  ## limit upload file size on server (10MB)
  options(shiny.maxRequestSize = getOption("dabtab.maxRequestSize", default = 10 * 1024^2))
...
## encoding
options(dabtab.encoding = "UTF-8")

## hack for rmarkdown from Report > Rmd and Report > R
options(dabtab.rmarkdown = FALSE)

## path to use for local or server use
options(
  dabtab.path.data =
    ifelse(grepl("dabtab.data", getwd()) && file.exists("../../inst"), "..", system.file(package = "dabtab.data"))
)

## import required functions and packages
## if dabtab.data is not in search main function from dplyr etc. won't be available
if (!"package:dabtab.data" %in% search() &&
  # isTRUE(Sys.getenv("SHINY_PORT") == "") &&
  isTRUE(getOption("dabtab.development")) &&
  getOption("dabtab.path.data") == "..") {
  import_fs("dabtab.data", libs = c("magrittr", "ggplot2", "lubridate", "tidyr", "dplyr", "broom", "tibble", "glue"))
...
##FOR VIEW STATE WILL BE REPLACED WITH TRACK CHANGES WHICH RETURN THE LOG CHANGE IN THE TEXT FORMAT

##HERE ALSO CAN APPLY TREE GRAPH AS EACH NOTE WILL BE REPRESENTED WITH EACH BRANCH


#THIS FUNCTION IS FOR RECORDING CHANGES 
recorded_actions <- reactiveVal(list())

# Function to record changes made by users
record_changes <- function(action, detail) {
  captured_data <- list(
    timestamp = Sys.time(),
    action = action,
    detail = detail
  )
  recorded_actions(c(recorded_actions(), captured_data))
}



#THIS FUNCTION IS FOR FINDING MATCHING STRING
find_and_append_match <- function(template, word) {
  # Search for a word within the template
  match_index <- grep(paste0("\\b", word, "\\b"), template)

  if (length(match_index) > 0) {
    # Append the word if found in the template
    # For example, adding word to the first match found
    template[match_index[1]] <- paste(template[match_index[1]], word)
  }
...
addResourcePath("www", file.path(getOption("dabtab.path.data"), "app/www/"))
addResourcePath("figures", file.path(getOption("dabtab.path.data"), "app/tools/help/figures/"))
addResourcePath("imgs", file.path(getOption("dabtab.path.data"), "app/www/imgs/"))
addResourcePath("js", file.path(getOption("dabtab.path.data"), "app/www/js/"))
...
if (Sys.info()["sysname"] != "Linux" && nzchar(local_mathjax)) {
  ...
  options(dabtab.mathjax.path = "latest")
  ...
  options(dabtab.mathjax.path = "https://mathjax.rstudio.com/latest")
}
rm(local_mathjax)
...
get_zip_info <- function() {
  ...
  options(dabtab.zip = c(flags, zip_util))
}
get_zip_info()
rm(get_zip_info)
...
help_menu <- function(hlp) {
  tagList(
    navbarMenu(
      ...
      tabPanel(tags$a(
        "",
        href = "https://dabtab-rstats.github.io/docs/tutorials.html", target = "_blank",
        list(icon("film", verify_fa = FALSE), "Videos")
      )),
      tabPanel("About", uiOutput("help_about"), icon = icon("info", verify_fa = FALSE)),
      tabPanel(tags$a(
        "",
        href = "https://dabtab-rstats.github.io/docs/", target = "_blank",
        list(icon("globe", verify_fa = FALSE), "Radiant docs")
      )),
      tabPanel(tags$a(
        "",
        href = "https://github.com/dabtab-rstats/dabtab/issues", target = "_blank",
...
## copy-right text
options(dabtab.help.cc = "&copy; Vincent Nijs (2023) <a rel='license' href='http://creativecommons.org/licenses/by-nc-sa/4.0/' target='_blank'><img alt='Creative Commons License' style='border-width:0' src ='imgs/by-nc-sa.png' /></a></br>")
options(dabtab.help.cc.sa = "&copy; Vincent Nijs (2023) <a rel='license' href='http://creativecommons.org/licenses/by-sa/4.0/' target='_blank'><img alt='Creative Commons License' style='border-width:0' src ='imgs/by-sa.png' /></a></br>")
...
make_url_patterns <- function(url_list = getOption("dabtab.url.list"),
                              url_patterns = list()) {
  for (i in names(url_list)) {
    res <- url_list[[i]]
    if (!is.list(res)) {
      url_patterns[[res]] <- list("nav_dabtab" = i)
    } else {
      tabs <- names(res)
      for (j in names(res[[tabs]])) {
        url <- res[[tabs]][[j]]
        url_patterns[[url]] <- setNames(list(i, j), c("nav_dabtab", tabs))
      }
    }
  }
  url_patterns
...
tmp <- grep("dabtab.", installed.packages()[, "Package"], value = TRUE)
if ("dabtab" %in% installed.packages()) {
  tmp <- c("dabtab" = "dabtab", tmp)
}

dabtab.versions <- "Unknown"
if (length(tmp) > 0) {
  dabtab.versions <- sapply(names(tmp), function(x) paste(x, paste(packageVersion(x), sep = ".")))
}

options(dabtab.versions = paste(dabtab.versions, collapse = ", "))
rm(tmp, dabtab.versions)


if (is.null(getOption("dabtab.theme", default = NULL))) {
  options(dabtab.theme = bslib::bs_theme(version = 4))
}

## bslib theme and version
has_bslib_theme <- function() {
  if (rlang::is_installed("bslib")) bslib::is_bs_theme(getOption("dabtab.theme")) else FALSE
}

bslib_current_version <- function() {
  if (rlang::is_installed("bslib")) bslib::theme_version(getOption("dabtab.theme", default = bslib::bs_theme(version = 4)))
}

navbar_proj <- function(navbar) {
  pdir <- dabtab.data::find_project(mess = FALSE)
  if (dabtab.data::is.empty(pdir)) {
    if (getOption("dabtab.shinyFiles", FALSE) && !dabtab.data::is.empty(getOption("dabtab.sf_volumes", ""))) {
      proj <- paste0("Base dir: ", names(getOption("dabtab.sf_volumes"))[1])
    } else {
      proj <- "Project: (None)"
    }
    options(dabtab.project_dir = NULL)
  } else {
    proj <- paste0("Project: ", basename(pdir)) %>%
      {
        if (nchar(.) > 35) paste0(strtrim(., 31), " ...") else .
      }
    options(dabtab.project_dir = pdir)
    options(dabtab.launch_dir = pdir)
  }
...
if (getOption("dabtab.shinyFiles", FALSE)) {
  if (!dabtab.data::is.empty(getOption("dabtab.sf_volumes", "")) && dabtab.data::is.empty(getOption("dabtab.project_dir"))) {
    launch_dir <- getOption("dabtab.launch_dir", default = dabtab.data::find_home())
    if (!launch_dir %in% getOption("dabtab.sf_volumes", "")) {
      sf_volumes <- c(setNames(launch_dir, basename(launch_dir)), getOption("dabtab.sf_volumes", ""))
      options(dabtab.sf_volumes = sf_volumes)
      rm(sf_volumes)
    } else if (!launch_dir == getOption("dabtab.sf_volumes", "")[1]) {
      dir_ind <- which(getOption("dabtab.sf_volumes") == launch_dir)[1]
      options(dabtab.sf_volumes = c(getOption("dabtab.sf_volumes")[dir_ind], getOption("dabtab.sf_volumes")[-dir_ind]))
      rm(dir_ind)
    }
    rm(launch_dir)
  }
  if (dabtab.data::is.empty(getOption("dabtab.launch_dir"))) {
    if (dabtab.data::is.empty(getOption("dabtab.project_dir"))) {
      options(dabtab.launch_dir = dabtab.data::find_home())
      options(dabtab.project_dir = getOption("dabtab.launch_dir"))
    } else {
      options(dabtab.launch_dir = getOption("dabtab.project_dir"))
    }
  }
  ...
  dbdir <- try(dabtab.data::find_dropbox(), silent = TRUE)
  dbdir <- if (inherits(dbdir, "try-error")) "" else paste0(dbdir, "/")
  options(dabtab.dropbox_dir = dbdir)
  rm(dbdir)

  gddir <- try(dabtab.data::find_gdrive(), silent = TRUE)
  gddir <- if (inherits(gddir, "try-error")) "" else paste0(gddir, "/")
  options(dabtab.gdrive_dir = gddir)
  rm(gddir)
} else {
  options(dabtab.launch_dir = dabtab.data::find_home())
  options(dabtab.project_dir = getOption("dabtab.launch_dir"))
  ...
  dabtab.shared_ui =
    tagList(
      ...
      navbarMenu("",
        ...
        "Server",
        tabPanel(actionLink("state_save_link", "Save dabtab state file", icon = icon("download", verify_fa = FALSE))),
        tabPanel(actionLink("state_load_link", "Load dabtab state file", icon = icon("upload", verify_fa = FALSE))),
        tabPanel(actionLink("state_share", "Share dabtab state", icon = icon("share", verify_fa = FALSE))),
        tabPanel("View dabtab state", uiOutput("state_view"), icon = icon("user", verify_fa = FALSE)),
        "----", "Local",
        tabPanel(downloadLink("state_download", tagList(icon("download", verify_fa = FALSE), "Download dabtab state file"))),
        tabPanel(actionLink("state_upload_link", "Upload dabtab state file", icon = icon("upload", verify_fa = FALSE)))
      ),
      ...
      navbarMenu("",
        ...
        tabPanel(tags$a(
          id = "refresh_dabtab", href = "#", class = "action-button",
...
onStop(function() {
  ...
  if (!exists("r_data")) {
    ...
    options(dabtab.launch_dir = NULL)
    options(dabtab.project_dir = NULL)
...
>>>>FOR REPORT
I WILL PROVIDE TEMPLATE<THESE TEMPLATES MIGHT BE REFERENCE FROM OTHER SOURCES>


 OR YOU CAN CREATE TEMPLATE YOURSELF



 =>SHOULD NOT BE SEPARATE EXPORT REPORTSR