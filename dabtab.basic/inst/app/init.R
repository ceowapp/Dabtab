...
#local save session
saveSessionLocal <- function(session = session, timestamp = FALSE, path = "~/.dabtab.sessions") {
...
#cloud save session
saveSessionCloud <- function(session = session, timestamp = FALSE, path = "~/.dabtab.sessions") {
  if (!exists("r_sessions")) {
    return()
  }
  if (!dir.exists(path)) dir.create(path)
...
observeEvent(input$refresh_dabtab, {
  if (isTRUE(getOption("dabtab.local"))) {
    fn <- normalizePath("~/.dabtab.sessions")
    file.remove(list.files(fn, full.names = TRUE))
  } else {
    fn <- paste0(normalizePath("~/.dabtab.sessions"), "/r_", r_ssuid, ".state.rda")
...
saveStateOnRefresh <- function(session = session) {
  session$onSessionEnded(function() {
    isolate({
      ...
      if (not_pressed(input$refresh_dabtab) &&
        not_pressed(input$stop_dabtab) &&
...
################################################################
## functions used across tools in dabtab
################################################################
...
# options(dabtab.autosave = list(1, 5, "~/.dabtab.sessions"))
# options(dabtab.autosave = list(.1, 1, "~/Desktop/dabtab.sessions"))
# options(dabtab.autosave = list(10, 180, "~/Desktop/dabtab.sessions"))



# Function to save the current session data
saveSession <- function(session, timestamp, autosave_path) {
  # Logic to save the current session data to a file or database
  # The session data is typically stored in a way that allows you to reconstruct the state later
  # Use appropriate file I/O, database queries, or any other storage mechanism
  # Example: (using file I/O)
  saveRDS(session$data, file = paste0(autosave_path, "/session_data_", timestamp, ".rds"))
}

# Function to recover the latest saved session data
recoverSession <- function(autosave_path) {
  # Logic to retrieve the latest saved session data
  # Search for the latest session data file and load it
  # Example: (using file I/O)
  all_files <- list.files(autosave_path, full.names = TRUE)
  if (length(all_files) > 0) {
    latest_file <- all_files[which.max(file.mtime(all_files))]
    recovered_session <- readRDS(latest_file)
...
observeEvent(input$enableAutoSave, {
  req(input$autoSaveTime)
  if (isTRUE(getOption("dabtab.autosave", default = FALSE))) {
    autoSave(input$autoSaveTime)
  }
})

autoSave <- function(saveTime) {
  observe({
    interval <- saveTime * 60  # Convert minutes to seconds
    autosave_path <- "~/.dabtab.sessions.autosave"  # Set the autosave path

    reactiveTimer(interval, session = shiny::getDefaultReactiveDomain()) {
      # Get the current timestamp for the session data
      timestamp <- format(Sys.time(), "%Y%m%d_%H%M%S")

      # Save the session data at the specified time interval
      saveSession(session, timestamp, autosave_path)
      message("Dabtab state was auto-saved at ", timestamp)
    }
  })
}

observeEvent(input$recoverSession, {
  if (isTRUE(getOption("dabtab.autosave", default = FALSE))) {
    recovered_session <- recoverSession("~/.dabtab.sessions.autosave")  # Pass the autosave path
    # Use the recovered_session to restore the session data
    if (!is.null(recovered_session)) {
      # Logic to restore the session data
      # For example:
      session$data <- recovered_session
      # Update your session variables or data accordingly
    }
  }
})
...
function autoSave (savetime)<-
if (length(getOption("dabtab.autosave", default = FALSE)) > 0) {
  start_time <- Sys.time()
  autosave_path <- ifelse(length(autosave_path) == 0, "~/.dabtab.sessions.autosave", autosave_path)
  autosave_poll <- reactivePoll(
    ...
    checkFunc = function() {
      ...
      if (diff_time < max_duration) {
        ...
        options(dabtab.autosave = list(interval, max_duration - diff_time, autosave_path))
        message("Dabtab state was auto-saved at ", curr_time)
      } else {
        if (length(getOption("dabtab.autosave", default = FALSE)) > 0) {
          
          options(dabtab.autosave = NULL)
        }
      }
    },
    valueFunc = function() {
...
# Define a global variable to store the change records
change_list <- list()

# Function to append a new change to the list
record_change <- function(action, detail) {
  new_change <- list(action = action, detail = detail)
  change_list <<- c(change_list, list(new_change))
}

# Function to step backward through changes
stepBackward <- function(step) {
  if (step == 1) {
    if (length(change_list) > 0) {
      change_list <<- change_list[-length(change_list)]
    }
  } else if (step == "clear_all") {
    change_list <<- list()
  }
}

# Function to step forward through changes
stepForward <- function(step) {
  if (step == 1) {
    # Implement the logic to go forward if required
  } else if (step == "recover_all") {
    # Implement the logic to recover all changes if required
  }
}

# Function to clear all change records on session end
...
dabtab_screenshot_modal <- function(report_on = "") {
  add_button <- function() {
...
download_handler_screenshot <- function(path, plot, ...) {
  ...
  if (inherits(plot, "try-error") || is.character(plot) || is.null(plot)) {
    ...
    ppath <- parse_path(path, pdir = getOption("dabtab.launch_dir", find_home()), mess = FALSE)
    # r_info[["latest_screenshot"]] <- glue("![]({ppath$rpath})")
    # r_info[["latest_screenshot"]] <- glue("<details>\n<summary>Click to show screenshot</summary>\n<img src='{ppath$rpath}' alt='Dabtab screenshot'>\n</details>")
    r_info[["latest_screenshot"]] <- glue("\n<details>\n<summary>Click to show screenshot with Dabtab settings to generate output shown below</summary>\n\n![]({ppath$rpath})\n</details></br>\n")
    png::writePNG(plot, path, dpi = 144)
  }
}

observe({
  if (length(input$nav_dabtab) > 0) {
    tabset <- names(getOption("dabtab.url.list")[[input$nav_dabtab]])
    rtn <- if (length(tabset) > 0) {
      paste0(input$nav_dabtab, " ", input[[tabset]])
    } else {
      input$nav_dabtab
    }
    r_info[["dabtab_tab_name"]] <- gsub("[ ]+", "-", rtn) %>%
      gsub("(\\(|\\))", "", .) %>%
      gsub("[-]{2,}", "-", .) %>%
      tolower()
  }
...
download_handler(
  ...
  fn = function() paste0(r_info[["dabtab_tab_name"]], "-screenshot"),
  type = "png",
  caption = "Save dabtab screenshot",
...
>>>when users upgrade from basic user to advanced user add navbar menu to allow access to more adavanced feature



>>>when save state that means freeze state at that particular moment current timestamp
