...
##Automatically save session every ten minutes 
##Session will be restore from where you left off at 10 minute intervals
setInterval(function() {
  Shiny.setInputValue('autosave', null);
...
saveSession <- function(session = session, timestamp = FALSE, path = "~/.dabtab.sessions") {
...
if (length(getOption("dabtab.autosave", default = NULL)) > 0) {
  start_time <- Sys.time()
  interval <- getOption("dabtab.autosave")[[1]] * 60000
  max_duration <- getOption("dabtab.autosave")[[2]]
  autosave_path <- getOption("dabtab.autosave")[[3]]
  autosave_path <- ifelse(length(autosave_path) == 0, "~/.dabtab.sessions", autosave_path)
          ...
          showModal(
            modalDialog(
              ...
              span(glue("The autosave feature has been turned off. Time to save and submit your work by clicking
                on the 'Save' icon in the navigation bar and then on 'Save dabtab state file'. To clean the
