...
FOR HELP FUNCTIONALITY INTEGRATE SUGGESTION FUNTIONALITY AS THAT OF EXCEL

SO FOR EACH FUNCTION THERE WILL BE A SUGGESTION LINK WHEN USERS CLICK ONTO THE LINK THEN SHOW THE HELP mainPanel


WHICH ESSENTIALLY INPUT/OUTPUT AND HOW TO USE
...
#MIGHT PROVIDE LINK TO YOUTUBE VIDEOS
output$help_guidance <- renderUI({
  file.path(getOption("radiant.path.data"), "app/tools/app/tutorials.md") %>% inclMD() %>% HTML()
})


#MIGHT PROVIDE LINK TO PERSONAL TRAINING 
output$help_training <- renderUI({
  file.path(getOption("radiant.path.data"), "app/tools/app/tutorials.md") %>% inclMD() %>% HTML()
})

#MIGHT PROVIDE LINK TO OTHER FORMS OF SUPPORT
output$help_support <- renderUI({
  file.path(getOption("radiant.path.data"), "app/tools/app/tutorials.md") %>% inclMD() %>% HTML()
})
...
#######################################
append_help <- function(help_str, help_path, lic = "nc", Rmd = TRUE) {
  ...
  if (lic == "nc") {
    ...
    cc <- getOption("radiant.help.cc.sa", default = "")
  }
...
output$help_data <- reactive(
  append_help("help_data", file.path(getOption("radiant.path.data"), "app/tools/help/"))
)
...
help_data_panel <-
  wellPanel(
    HTML(
      "<label>Data menu: <i id='help_data_all' title='Check all' href='#' class='action-button glyphicon glyphicon-ok'></i>
