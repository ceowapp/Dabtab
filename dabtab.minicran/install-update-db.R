...
build <- function(type = "binary") {
  ...
  # dabtab
  install.packages(c("dabtab", "miniUI", "webshot", "tinytex", "svglite", "readxl", "writexl"), Ncpus = -1)
  ...
  #remotes::install_github("dabtab-rstats/dabtab.update", upgrade = "never")
  #remotes::install_github("dabtab-rstats/dabtab.data", upgrade = "never")
  #remotes::install_github("DiagrammeR", upgrade = "never")
  #remotes::install_github("gitgadget", upgrade = "never")
  ...
  if (os == "Windows") {
    ...
    if (grepl("(Prog)|(PROG)", Sys.getenv("R_HOME"))) {
      ...
      if (!grepl("zip", wz) && !grepl("7-Zip", w7z)) {
        # URL <- "http://rady.ucsd.edu/faculty/directory/nijs/pub/docs/dabtab/7z1604-x64.exe"
      ...
      cat("\nTo generate PDF reports in dabtab you will need TinyTex (or MikTex).\n")
      inp <- readliner("Proceed with the download and install of TinyTex? Press y or n and then press return: ")
      if (grepl("[yY]", inp)) {
        tinytex::install_tinytex()
      }
      cat("\n\nInstallation on Windows complete. Close R, (re)start Rstudio, and select 'Start dabtab'\nfrom the Addins menu to get started\n\n")
    }
  } else if (os == "Darwin") {
    ...
    if (as.numeric_version(resp) < as.numeric_version("10.9")) {
      ...
      if (length(pl) == 0) {
        cat("To generate PDF reports in dabtab (Report > Rmd) you will need TinyTex (or MacTex).")
        inp <- readliner("Proceed with the TinyTex download and install? Press y or n and then press return: ")
        if (grepl("[yY]", inp)) {
          tinytex::install_tinytex()
        }
      }
      cat("\n\nInstallation on Mac complete. Close R, (re)start Rstudio, and select 'Start dabtab'\nfrom the Addins menu to get started\n\n")
    }
  } else {
    cat("\n\nThe install script only partially supports your OS\n")
    cat("You may prefer to use a docker image of dabtab and related software\nSee https://github.com/dabtab-rstats/docker/blob/master/install/rsm-msba-linux.md for details\n\n")
    inp <- readliner("Do wish to proceed with the local install of dabtab and its dependencies? Press y or n and then press return: ")
    if (grepl("[yY]", inp)) {
      build()
      pl <- suppressWarnings(system("which pdflatex", intern = TRUE))
      if (length(pl) == 0) {
        cat("To generate PDF reports in dabtab (Report > Rmd) you will need TinyTex (or LaTex).")
        inp <- readliner("Proceed with the TinyTex download and install? Press y or n and then press return: ")
        if (grepl("[yY]", inp)) {
          tinytex::install_tinytex()
        }
