...
build <- function(type = ifelse(os == "Linux", "source", "binary")) {
  ...
  install(c("dabtab", "miniUI", "webshot", "tinytex", "usethis", "svglite", "remotes"))

  if (!"dabtab.update" %in% ipkgs) {
    ap <- available.packages(repos = repos)[, "Package"]
    if ("dabtab.update" %in% ap) {
      install.packages("dabtab.update", lib = .libPaths()[1], repos = repos)
      dabtab.update::dabtab.update()
    } else {
      ru <- try(remotes::install_github("aigeniusxadmin/dabtdab.update", upgrade = "never"))
      if (inherits("try-error", ru)) {
        print("dabtab.update cannot be installed on your system at this time")
      } else {
        dabtab.update::dabtab.update()
      }
    }
...
if (rv < "3.6") {
  cat("dabtab requires R-3.6.0 or later. Please install the latest\nversion of R from https://cloud.r-project.org/")
} else {
  os <- Sys.info()["sysname"]
  if (os == "Windows") {
