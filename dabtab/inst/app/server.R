...
#########################################
##MORE ERRORS HANDLING WILL BE PUT FORTH
############################################
...
enc <- getOption("dabtab.encoding")

## source shared functions
source(file.path(getOption("dabtab.path.basic"), "app/init.R"), encoding = enc, local = TRUE)
source(file.path(getOption("dabtab.path.basic"), "app/dabtab.R"), encoding = enc, local = TRUE)


  ## source data & app tools from dabtab.data
  for (file in list.files(
    c(
      file.path(getOption("dabtab.path.data"), "app/tools/app"),
      file.path(getOption("dabtab.path.data"), "app/tools/data")
    ),
    pattern = "\\.(r|R)$",
    full.names = TRUE)) {
...
# Define the global database connection
global_db <- reactiveValues(
...
shinyServer(function(input, output, session) {
  ...
  summary.correlation <- dabtab.basics:::summary.correlation

  


  ## list of dabtab menu's to include
  rmenus <- c("dabtab.data", "dabtab.design", "dabtab.basics", "dabtab.model", "dabtab.multivariate")

  ## packages to use for example data
  options(dabtab.example.data = rmenus)

  for (i in rmenus[-1]) {
    ## 'sourcing' dabtab's package functions in the server.R environment
    if (!isTRUE(getOption("dabtab.from.package"))) {
      eval(parse(text = paste0("dabtab.data::copy_all(", i, ")")))
  ...
  source(file.path(getOption("dabtab.path.model"), "app/dabtab.R"), encoding = enc, local = TRUE)
